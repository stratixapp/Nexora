...
async function dbAddCategory({ name, slug, icon, image_url }) {
  const { data, error } = await sb
    .from("categories")
    .insert({ name, slug, icon: icon || "🛍️", image_url: image_url || null })
    .select()
    .single();
...
async function dbUpdateCategory(id, changes) {
...
/* ---------------- REVIEWS ---------------- */
async function dbGetReviews(productId) {
  if (!sb) return [];
  const { data, error } = await sb
    .from("reviews")
    .select("*")
...
async function dbAddReview({ product_id, rating, comment }) {
  const session = await authGetSession();
  if (!session) throw new Error("Sign in to write a review.");
  ...
  const { data, error } = await sb
    ...
    .insert({
      ...
      reviewer_name: profile?.full_name || session.user.email || "NEXORA customer",
      rating,
      comment: comment || "",
    })
    .select()
...
// Lightweight rating lookup for product grids — one query, computed client-side
// (small catalogs; avoids N queries per grid render).
async function dbGetAllReviewRatings() {
  if (!sb) return {};
  const { data, error } = await sb.from("reviews").select("product_id, rating");
  if (error) { console.error(error); return {}; }
  const map = {};
  for (const r of data) {
...
// Registers the service worker (app shell caching for the installed app /
// PWA). Safe no-op if the browser doesn't support it or this file is
// opened directly from disk (file://) during local preview.
