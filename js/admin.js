...
async function refreshCategories() {
  ...
  $("#categoryManager").innerHTML = categories.length
    ? categories
        .map(
          (c) =>
            `<div class="category-row"><span class="cinfo">${c.icon} ${c.name}</span><button class="danger" data-del-cat="${c.id}">Delete</button></div>`
        )
        .join("")
...
$("#categoryForm").onsubmit = async (e) => {
  ...
  try {
    await dbAddCategory({ name, slug, icon: f.get("icon") || "🛍️" });
    e.target.reset();
    await refreshCategories();
  } catch (err) {
