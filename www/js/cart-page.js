...
async function loadCartSizeGuide() {
  ...
  $("#cartSizeGuideContent").innerHTML = `<table class="size-guide-table">
...
$("#closeCartSizeGuide").onclick = () => $("#cartSizeGuideModal").classList.remove("open");
...
function renderItems() {
    ...
    .map(
      (i) => `<div class="cart-item">
        ...
        <div>
          ...
          <p class="ci-meta">${[i.unit, i.color ? `Colour: ${i.color}` : "", i.size ? `Size: ${i.size}` : ""].filter(Boolean).join(" · ")}${i.size ? ` <button type="button" class="size-guide-link" data-cart-size-guide>Size guide</button>` : ""}</p>
  ...
  box.querySelectorAll("[data-cart-size-guide]").forEach((b) => (b.onclick = async () => { $("#cartSizeGuideModal").classList.add("open"); await loadCartSizeGuide(); }));
}
    ...
    ${
      profile
        ...
        : `<div class="cart-summary" style="margin-bottom:14px">
            ...
            <a class="guest-nudge" href="account.html?next=cart.html">Create an account to save these details for next time →</a>
          </div>`
    }
